...
# Debian's postinst creates this at install time, but we add it here
...
# sed -i "s|\#*bind-address\s*=.*|bind_address = 127.0.0.1,::1,db,mariadb|g" /etc/mysql/mariadb.conf.d/50-server.cnf
if [ "${DISALLOW_ROOT_LOGIN_REMOTELY}" = "0" ] || [ "${DISALLOW_USER_LOGIN_REMOTELY}" = "0" ]; then
